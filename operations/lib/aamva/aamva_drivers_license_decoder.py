...
def decode_aamva_fields(pdf417_data_txt: typing.List[str]) -> dict:
    """
    pdf417_data_txt is a list of text lines consisting of the full
    PDF-417 data from the drivers license barcode
    """
...
if __name__ == "__main__":
    ...
    argparse_parser.add_argument(
        ...
        "--text-file",
        type=str,
        help="Input text file containing the decoded PDF-417 data, to read from",
        required=True,
    )
    argparse_parser.add_argument(
        "-o", "--output-file", type=str, help="Output text file to write results to"
    )
    ...
    if os.path.exists(input_filename):
        with open(file=input_filename, mode="r", encoding="utf-8") as input_file:
            ...
            if argparse_args.output_file is not None:
                with open(
                    file=argparse_args.output_file, mode="w", encoding="utf-8"
                ) as output_file:
