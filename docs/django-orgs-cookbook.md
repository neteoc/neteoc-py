...
This can be accomplished with proxy models in your models module (e.g., `models.py`):
...
We'll go through this piece by piece, but here's the full class (e.g., in `forms.py`):
...
## Custom Organization with Simple Inheritance
...
If you chose the "single table inheritance" approach, you may want to reuse the base admin classes too, in order to avoid having too much boilerplate code in your application:
...
The linking model should in _most scenarios_ enforce uniqueness against the linked resource model to prevent multiple organizations from having access to the resource.
...
The `for_user` method can then reduce the queryset to only documents belonging to _any_ organization which the current user is a member of.
